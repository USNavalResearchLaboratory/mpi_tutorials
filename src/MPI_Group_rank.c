...
Parameters
   ...
   rank
          [out] rank of the calling process in group, or MPI_UNDEFINED if
          the process is not a member (integer)
...
int main( int argc, char **argv )
{
    ...
    if (result != MPI_UNEQUAL) {
        errs++;
        fprintf( stdout, "Group compare of equal sized but different groups"
		 "should have been unequal, was %d\n", result );
	fflush(stdout);
    }
